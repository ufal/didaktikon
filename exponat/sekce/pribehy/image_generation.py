...
def _get_image_for_line(line, seed):
    try:
        filename = text2id(line) + '_' + str(seed) + ".png"
        filename_full = f'{IMGDIR}/{filename}'
        if not os.path.isfile(filename_full):
            generate_image(line, seed, filename_full)
    except Exception as e:
        message = f'StoryBoarder: Cannot generate image "{filename}" for "{line}": {e}'
        logging.warning(message)
        print(message)
        filename = "BACKUP_IMAGE.png"
...
if __name__=="__main__":
    ...
    print(f"{IMGDIR}/{get_image_for_line(line, 1987)}")
